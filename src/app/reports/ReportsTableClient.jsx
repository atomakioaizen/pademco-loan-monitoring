      ...
      return dataToRender.map((l) => {
        ...
        return (
          <tr key={l.id} className={`hover:bg-slate-50 transition-colors font-mono text-[11px] divide-x divide-slate-100 ${isOld ? "bg-amber-50/20" : ""}`}>
            ...
            <td className="px-2 py-2 text-right whitespace-nowrap">{l.dr.toLocaleString("en-US", { minimumFractionDigits: 2 })}</td>
            <td className="px-2 py-2 text-right text-slate-600 whitespace-nowrap">{l.markup > 0 ? l.markup.toLocaleString("en-US", { minimumFractionDigits: 2 }) : "—"}</td>
            <td className="px-2 py-2 text-right font-bold text-blue-900 bg-blue-50/30 whitespace-nowrap">{l.totalTicket.toLocaleString("en-US", { minimumFractionDigits: 2 })}</td>
            ...
            <td className="px-2 py-2 text-right font-bold text-emerald-800 bg-emerald-50/30 whitespace-nowrap">{l.totalPaid.toLocaleString("en-US", { minimumFractionDigits: 2 })}</td>
            <td className={`px-2.5 py-2 text-right font-black whitespace-nowrap min-w-[110px] ${l.unpaidBalance > 0 ? "text-rose-700 bg-rose-50/40" : "text-slate-500"}`}>
              {l.unpaidBalance.toLocaleString("en-US", { minimumFractionDigits: 2 })}
            </td>
          </tr>
        );
      });
    ...
    if (reportType === "unpaid_schedule") {
      return (
        <tr className="bg-slate-100 text-slate-800 font-extrabold text-[10px] uppercase tracking-wider divide-x divide-slate-200">
          ...
          <th scope="col" className="px-2 py-2.5 text-right bg-blue-50/70 text-blue-900 min-w-[95px]">TOTAL TICKET</th>
          ...
          <th scope="col" className="px-2 py-2.5 text-right bg-emerald-50/70 text-emerald-900 min-w-[90px]">TOTAL PAID</th>
          <th scope="col" className="px-2.5 py-2.5 text-right bg-rose-50/70 text-rose-900 font-black min-w-[110px] whitespace-nowrap">UNPAID BALANCE</th>
        </tr>
      );
    }
    if (reportType === "outstanding") {
