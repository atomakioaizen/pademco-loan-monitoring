      ...
      <div className="bg-white rounded-2xl shadow-sm border border-amber-200 p-6 space-y-5 self-start">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className="w-full bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white py-3 rounded-xl text-xs font-black shadow-md transition-all cursor-pointer flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
                ...
                <span>Processing Old Loan Payment...</span>
              </>
            ) : (
              `Post ${paymentType === "FULL" ? "Full" : "Partial"} Payment →`
            )}
          </button>
        </form>
      </div>
