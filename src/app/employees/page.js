  ...
  const searchWhere = search
    ? {
        OR: [
          ...
          { govIdNumber: { contains: search } },
          { govIdType: { contains: search } },
        ],
      }
    : {};
  ...
  const [
    ...
    viewId
      ? db.employee.findUnique({
          where: { id: viewId },
          include: { office: true, user: { include: { creator: true } } },
        })
    ...
    try {
      ...
      if (id) {
        ...
        if (!empUser) {
          ...
          await logAction(session.id, "CREATE", "USER", `[Account: ${fullName} (${username})] Account generated by ${session.role} (${session.name || session.username}).`);
        } else {
        ...
        await logAction(session.id, "UPDATE", "EMPLOYEE", `[Account: ${emp.fullName}] Updated employee details (ID: ${emp.employeeId}).`);
      } else {
        ...
        await logAction(session.id, "CREATE", "EMPLOYEE", `[Account: ${emp.fullName}] Registered employee profile (ID: ${emp.employeeId}) and generated portal account by ${session.role}.`);
      }
    } catch (e) {
              ...
              <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                ...
                <div className="mt-4 pt-4 border-t border-slate-100">
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 space-y-4">
                    ...
                    {viewEmployee.user && (
                      <div className="bg-blue-50/70 border border-blue-200 rounded-xl p-3 text-xs flex flex-wrap items-center justify-between gap-2">
                        <div>
                          <span className="font-bold text-blue-950 block">📅 Created: {viewEmployee.user.createdAt ? new Date(viewEmployee.user.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "N/A"}</span>
                          <span className="text-[11px] text-blue-800 font-semibold mt-0.5 block">
                            👤 Created By: <strong className="font-extrabold uppercase">{viewEmployee.user.createdByRole || (viewEmployee.user.creator ? viewEmployee.user.creator.role : "ADMIN")}</strong>
                            {viewEmployee.user.creator ? ` (${viewEmployee.user.creator.name})` : viewEmployee.user.createdByRole === "LOANER" ? " (Self-Registered)" : ""}
                          </span>
                        </div>
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-black uppercase border bg-white text-blue-800 border-blue-300">
                          {viewEmployee.user.status}
                        </span>
                      </div>
                    )}
