  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-200 bg-slate-50/50 p-2 gap-2 rounded-2xl shadow-sm border select-none">
        ...
        <button
          onClick={() => setActiveTab("DECISIONS_LOG")}
          className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-xs font-black transition-all cursor-pointer border select-none ${
            activeTab === "DECISIONS_LOG"
              ? "bg-white text-primary border-slate-250 shadow-sm"
              : "text-slate-500 hover:text-slate-800 hover:bg-slate-100/50 border-transparent"
          }`}
        >
          <span>📜 Decisions & Remarks Log</span>
          <span className="bg-slate-200 text-slate-700 rounded-full px-2 py-0.5 text-[10px] font-bold font-mono">
            {approvalAuditLogs.length}
          </span>
        </button>
      </div>
      ...
      {/* TAB 4: PERMANENT DECISIONS & REMARKS LOG */}
      {activeTab === "DECISIONS_LOG" && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h3 className="text-lg font-black text-slate-800">Permanent Decisions & Remarks Audit History</h3>
              <p className="text-xs text-slate-400 mt-1">
                Permanent recorded log of all override approvals, rejections, remarks, user actions, and modifications across the system.
              </p>
            </div>
            ...
            <div className="relative min-w-[280px]">
              <input
                type="text"
                placeholder="Search borrower, remarks, reviewer, action..."
                value={historySearch}
                onChange={(e) => setHistorySearch(e.target.value)}
                className="w-full rounded-xl border border-slate-300 px-4 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-primary/20 text-slate-900 font-medium shadow-sm bg-white"
              />
            </div>
          </div>

          {(() => {
            const filteredAudit = approvalAuditLogs.filter((log) => {
              const query = historySearch.toLowerCase().trim();
              if (!query) return true;
              const actor = (log.user?.name || "").toLowerCase();
              const details = (log.details || "").toLowerCase();
              const action = (log.action || "").toLowerCase();
              const resource = (log.resource || "").toLowerCase();
              return actor.includes(query) || details.includes(query) || action.includes(query) || resource.includes(query);
            });
            ...
            if (filteredAudit.length === 0) {
              return (
                <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 text-xs font-medium">
                  No decision logs match your query.
                </div>
              );
            }
                ...
                <table className="min-w-full divide-y divide-slate-200 text-left text-xs">
                  <thead className="bg-slate-50 text-slate-500 font-bold">
                    <tr>
                      ...
                      <th className="px-4 py-3">Reviewer / Actor</th>
                      <th className="px-4 py-3">Action Badge</th>
                      <th className="px-4 py-3">Module</th>
                      <th className="px-4 py-3">Decision Remarks & Detailed History Log</th>
                    </tr>
                  </thead>
                    ...
                    {filteredAudit.map((log) => (
                      <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3.5 whitespace-nowrap font-bold text-slate-800">
                          ...
                          <span className="block text-[9px] font-normal text-slate-400 uppercase">{log.user?.role || "SYSTEM"}</span>
                        </td>
                        <td className="px-4 py-3.5 whitespace-nowrap">
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-black border uppercase tracking-wider ${
                            log.details.includes("APPROVED") || log.details.includes("approved")
                              ? "bg-emerald-100 text-emerald-800 border-emerald-200"
                              : log.details.includes("REJECTED") || log.details.includes("declined") || log.action === "DELETE"
                        ...
                        <td className="px-4 py-3.5 whitespace-nowrap font-bold text-slate-500 text-[10px] uppercase">
                          {log.resource}
                        </td>
                        <td className="px-4 py-3.5 text-slate-700 font-sans leading-relaxed">
                          {log.details}
                        </td>
                      </tr>
                    ))}
